...
class _EditMovieScreenState extends ConsumerState<EditMovieScreen> {
  ...
  void _loadMovie() {
    WidgetsBinding.instance.addPostFrameCallback((_) {
      ...
      final movie = movieState.movies.where((m) => m.id.toString() == widget.movieId).firstOrNull;
      if (movie != null && mounted) {
        _movie = movie;
        _titleCtrl.text = movie.title;
        _genreCtrl.text = movie.genre;
        _imdbCtrl.text = movie.imdbId ?? '';
        _tmdbCtrl.text = movie.tmdbId ?? '';
        _overviewCtrl.text = movie.description ?? '';
        _castCtrl.text = movie.cast ?? '';
        _castPhotosCtrl.text = movie.castPhotos ?? '';
        _directorCtrl.text = movie.director ?? '';
        _directorPhotoCtrl.text = movie.directorPhoto ?? '';
        _releaseCtrl.text = movie.releaseDate ?? '';
        _runtimeCtrl.text = movie.runtime ?? '';
        _trailerCtrl.text = movie.trailerUrl ?? '';
        _quality = movie.qualityTag ?? '1080p';
        _posterCtrl.text = movie.posterUrl;
        _backdropCtrl.text = movie.backdropUrl ?? '';
        _logoCtrl.text = movie.logoUrl ?? '';
        _posterPreviewUrl = movie.posterUrl.isNotEmpty ? movie.posterUrl : null;
        _backdropPreviewUrl = movie.backdropUrl?.isNotEmpty == true ? movie.backdropUrl : null;
        _logoPreviewUrl = movie.logoUrl?.isNotEmpty == true ? movie.logoUrl : null;
        _languageId = movie.languageId > 0 ? movie.languageId : null;
        _ottId = movie.ottId > 0 ? movie.ottId : null;
        if (movie.createdAt != null && movie.createdAt!.isNotEmpty) {
          _ingestionDate = DateTime.tryParse(movie.createdAt!.replaceAll(' ', 'T'));
        }
        // Parse stream sources from JSON
  ...
  Future<void> _loadDropdowns() async {
    try {
      ...
      if (mounted) setState(() { _languages = langs; _ottProviders = otts; _loadingDropdowns = false; });
    } catch (_) { if (mounted) setState(() => _loadingDropdowns = false); }
  }

  void _onTmdbSearchChanged(String query) {
    _debounce?.cancel();
    if (query.isEmpty) { setState(() { _tmdbSuggestions = []; _isSearchingTMDB = false; }); return; }
  ...
  Future<void> _selectTmdbMovie(int movieId) async {
    ...
    } else if (mounted) setState(() { _isSearchingTMDB = false; _tmdbSuggestions = []; });
  }

  void _addStreamSource({String name = 'mp4/mkv', String url = ''}) {
    setState(() { _streamSources.add(_StreamSource(id: _sourceIdCounter++, nameCtrl: TextEditingController(text: name), urlCtrl: TextEditingController(text: url))); });
  }

  void _removeStreamSource(int id) { setState(() => _streamSources.removeWhere((s) => s.id == id)); }
  ...
  Future<void> _testSourceHealth(int id) async {
    ...
    try {
      ...
      if (url.contains('youtube.com') || url.contains('youtu.be')) {
        try { final res = await http.head(Uri.parse(url)); source.status = res.statusCode < 400 ? 'Active' : 'Broken'; } catch (_) { source.status = 'Broken'; }
      } else if (url.startsWith('http')) {
        try { final res = await http.head(Uri.parse(url), headers: {'User-Agent': 'Mozilla/5.0'}); source.status = res.statusCode < 400 || res.statusCode == 403 ? 'Active' : 'Broken'; } catch (_) {
          try { final res = await http.get(Uri.parse(url), headers: {'User-Agent': 'Mozilla/5.0', 'Range': 'bytes=0-0'}); source.status = res.statusCode == 206 || res.statusCode == 200 ? 'Active' : 'Broken'; } catch (_) { source.status = 'Broken'; }
        }
      } else if (url.startsWith('magnet:')) { source.status = 'Active'; }
      else if (url.startsWith('stalker://')) { source.status = 'Active'; }
      else { source.status = 'Unknown'; }
    } finally { if (mounted) setState(() => source.isTesting = false); }
  }
  ...
  Future<void> _save() async {
    ...
    try {
      final streamSourcesJson = _streamSources.where((s) => s.urlCtrl.text.trim().isNotEmpty).map((s) => '{"name":"${s.nameCtrl.text.trim()}","url":"${s.urlCtrl.text.trim()}"}').join(',');
      final data = <String, dynamic>{
        'title': _titleCtrl.text, 'description': _overviewCtrl.text, 'language_id': _languageId?.toString() ?? '',
        'release_date': _releaseCtrl.text, 'runtime': _runtimeCtrl.text, 'trailer_url': _trailerCtrl.text,
        'quality_tag': _quality, 'poster_url': _posterCtrl.text, 'backdrop_url': _backdropCtrl.text, 'logo_url': _logoCtrl.text,
        'tmdb_id': _tmdbCtrl.text, 'imdb_id': _imdbCtrl.text, 'cast': _castCtrl.text, 'cast_photos': _castPhotosCtrl.text,
      ...
      if (mounted) {
        ScaffoldMessenger.of(context).showSnackBar(SnackBar(
          content: Text(result['message'] ?? 'Movie updated'),
  ...
  Widget build(BuildContext context) {
    if (_isLoading) return Scaffold(appBar: AppBar(title: const Text('Edit Movie')), body: const LoadingWidget(message: 'Loading...'));
    if (_movie == null) return Scaffold(appBar: AppBar(title: const Text('Edit Movie')), body: const Center(child: Text('Movie not found', style: TextStyle(color: Colors.white54))));

    return Scaffold(
      appBar: AppBar(leading: IconButton(icon: const Icon(Icons.menu), onPressed: DrawerProvider.openDrawer), title: const Text('Edit Movie')),
      body: Form(
        key: _formKey,
        child: ListView(
          ...
          children: [
                ...
                child: Container(
                  ...
                  child: DropdownButtonHideUnderline(
                    child: DropdownButton<String>(
                      value: _quality, dropdownColor: const Color(0xFF1A1F2E), isExpanded: true,
            ...
            const SizedBox(height: 16),
            // Stream status
            if (_movie!.streamStatus != null)
              Container(
                padding: const EdgeInsets.all(12),
                margin: const EdgeInsets.only(bottom: 12),
                decoration: BoxDecoration(
                  color: (_movie!.isBroken ? Colors.red : Colors.green).withOpacity(0.1),
                  borderRadius: BorderRadius.circular(12),
                  border: Border.all(color: (_movie!.isBroken ? Colors.red : Colors.green).withOpacity(0.3)),
                ),
                child: Row(children: [
                  Icon(_movie!.isBroken ? Icons.error : Icons.check_circle, color: _movie!.isBroken ? Colors.red : Colors.green, size: 18),
                  const SizedBox(width: 8),
                  Text('Stream: ${_movie!.isBroken ? "Broken" : "Active"}', style: TextStyle(color: _movie!.isBroken ? Colors.red : Colors.green, fontWeight: FontWeight.bold, fontSize: 13)),
                  const Spacer(),
                  if (_movie!.lastChecked != null) Text('Checked: ${_movie!.lastChecked}', style: TextStyle(color: Colors.white54, fontSize: 10)),
                ]),
              ),
            // Stream Sources
                ...
                child: Column(
                  children: [
                    Row(children: [
                        ...
                        child: Container(
                          ...
                          child: DropdownButtonHideUnderline(
                            child: DropdownButton<String>(
                              ...
                              dropdownColor: const Color(0xFF1A1F2E), isExpanded: true,
                      ...
                      const SizedBox(width: 8),
                      Expanded(flex: 2, child: _field('URL', s.urlCtrl, Icons.link, small: true)),
                    ]),
                    ...
                    Row(mainAxisAlignment: MainAxisAlignment.end, children: [
                        ...
                        Padding(
                          ...
                          child: Row(mainAxisSize: MainAxisSize.min, children: [
                            Container(width: 6, height: 6, decoration: BoxDecoration(
                              color: s.status == 'Active' ? const Color(0xFF10B981) : s.status == 'Broken' ? const Color(0xFFEF4444) : Colors.grey, shape: BoxShape.circle,
                            )),
                      ...
                      SizedBox(height: 28, width: 28,
                        child: IconButton(
                          ...
                          icon: s.isTesting ? const SizedBox(width: 14, height: 14, child: CircularProgressIndicator(strokeWidth: 2)) : const Icon(Icons.favorite_border, color: Colors.white54),
        ...
        child: Padding(
          ...
          child: Row(children: [
            ClipRRect(
              ...
              child: m.posterPath != null ? Image.network(m.posterPath!, width: 36, height: 54, fit: BoxFit.cover, errorBuilder: (_, __, ___) => Container(width: 36, height: 54, color: Colors.grey[850])) : Container(width: 36, height: 54, color: Colors.grey[850], child: const Icon(Icons.movie, size: 18, color: Colors.white24)),
            ),
            const SizedBox(width: 12),
  ...
  Widget _dropdown(String label, List<Map<String, dynamic>> items, int? value, ValueChanged<int?> onChanged, {bool required = false}) {
    return Container(
      ...
      child: DropdownButtonHideUnderline(child: DropdownButton<int>(
        ...
        items: (required ? items : [<String, dynamic>{'id': 0, 'name': 'None'}, ...items]).map((e) => DropdownMenuItem(
        ...
        onChanged: onChanged, style: const TextStyle(color: Colors.white, fontSize: 14),
        icon: Icon(Icons.arrow_drop_down, color: Colors.white.withOpacity(0.5)),
      )),
    );
  }
  ...
  Widget _imageField(String label, TextEditingController ctrl, bool isPoster) {
    ...
    return Container(
      ...
      child: TextField(
        ...
        onChanged: (v) { setState(() { if (isPoster) _posterPreviewUrl = v.isNotEmpty ? v : null; else _backdropPreviewUrl = v.isNotEmpty ? v : null; }); },
        decoration: InputDecoration(
